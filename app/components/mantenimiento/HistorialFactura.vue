...
import type { ArchivoRow } from '~/composables/useMantenimientos'
...
async function cargar() {
  versiones.value = await listarHistorialFactura(props.mantenimientoId)
}
...
async function onSubirVersion() {
  ...
  errorSubida.value = ''
  try {
    await adjuntarFactura(props.mantenimientoId, archivoSeleccionado.value)
    cerrarDialogo()
    cargando.value = true
    await cargar()
    cargando.value = false
    ...
    errorSubida.value = errorMantenimientos.value ?? 'No se pudo subir la nueva factura.'
  } finally {
