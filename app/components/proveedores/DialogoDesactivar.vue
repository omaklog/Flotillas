<template>
  <v-dialog :model-value="modelValue" max-width="480" @update:model-value="onUpdateModelValue">
    <v-card class="app-modal-shadow" variant="flat">
      <v-card-title>Desactivar el proveedor</v-card-title>
      <v-card-text>
        <p class="text-body-main mb-4">
          El proveedor dejará de aparecer en el listado por defecto. Puedes reactivarlo cuando lo
          necesites.
        </p>
        <v-textarea
          v-model="motivo"
          ...
          counter="150"
          rows="3"
          data-testid="dialogo-desactivar-motivo"
        />
        <v-alert v-if="errorExterno" type="error" class="mt-2" data-testid="dialogo-desactivar-error">
          {{ errorExterno }}
        </v-alert>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
        <v-btn variant="text" data-testid="dialogo-desactivar-cancelar" @click="onCancelar">
          Cancelar
        </v-btn>
        <v-btn
          color="error"
          ...
          data-testid="dialogo-desactivar-confirmar"
          @click="onConfirmar"
        >
          Desactivar
        </v-btn>
