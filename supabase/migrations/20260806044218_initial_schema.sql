...
--
-- NOTA DE REORDENAMIENTO (Feature 001, /speckit-implement): el archivo
-- fuente en docs/schema-reference/schema.sql define las funciones
-- auxiliares de RLS (private.empresa_id(), private.rol(), etc.) ANTES de
-- crear la tabla `usuarios` a la que hacen referencia. Como son funciones
-- `language sql` (no `plpgsql`), Postgres las valida en el momento de
-- `CREATE FUNCTION`, no al ejecutarlas — con el orden original,
-- `supabase db reset` falla con "relation public.usuarios does not
-- exist". Esta migración aplica el mismo contenido, reordenado así:
--   1. Extensiones/enums (igual)
--   2. Tablas `empresas` y `usuarios` DESNUDAS (sin políticas RLS todavía)
--   3. Funciones auxiliares `private.*` (ya pueden referenciar `usuarios`)
--   4. Triggers + RLS + políticas de `empresas` y `usuarios`
--   5. Resto del esquema, igual que el original (ya venía después de
--      `usuarios`, así que no tenía este problema)
-- docs/schema-reference/schema.sql NO se modifica — sigue siendo la
-- referencia de diseño; este reordenamiento es un detalle de aplicación.
--
...
-- ---------------------------------------------------------------------
-- 1. Empresas y Usuarios — tablas desnudas primero (sin RLS todavía),
--    para que las funciones auxiliares de la sección 2 puedan
--    referenciar `usuarios` al crearse.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 2. Funciones auxiliares para RLS (schema "private", no expuesto por la API)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. Empresas — trigger + RLS + políticas (la tabla ya existe, sección 1)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 4. Usuarios — trigger + RLS + políticas (la tabla ya existe, sección 1)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 5. Tipos de vehículo (catálogo por empresa)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 9. Permisos (catálogo) y su relación con vehículos
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 11. Productos (catálogo por empresa)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 12. Cargas de combustible (INMUTABLE tras captura, solo cancelable)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13. Mantenimientos (INMUTABLE tras captura, solo cancelable)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 14. Checklist de aditamentos y revisión de seguridad
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 15. Bitácora de servicios obligatorios
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 16. Alertas (generadas por Edge Function / cron, con service role)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 17. Archivos (metadata; el binario vive en Supabase Storage)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 18. Auditoría (solo INSERT vía trigger de aplicación; nunca UPDATE/DELETE)
-- ---------------------------------------------------------------------
